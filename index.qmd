...
# Brief history of World Wide Web {background-image="./img/old-pc.jpg" background-position="center" background-opacity="0.4"}
...
# Wasm {background-image="./img/scrabble.jpg" background-position="center" background-opacity="0.4"}
...
- No access to [Document Object Model](https://developer.mozilla.org/en-US/docs/Web/API/Document_Object_Model) (DOM)
...
# Can I use? {background-image="./img/pottery.jpg" background-position="center" background-opacity="0.4"}

## Decision tree

```{mermaid}
flowchart LR
  software[Software] --> gui{GUI?}
  gui -- no --> language{C code?}

  language -- no --> wasm(Try Wasm)

  language -- yes --> conda{In Conda?}

  conda -- yes --> wasm
  conda -- no --> no
  
  gui -- yes --> no(Avoid Wasm)
```
